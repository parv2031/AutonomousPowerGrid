...
class PowerGridEnv(gym.Env):
    ...
    def _send(self, payload: dict) -> dict:
        ...
        On ZMQ errors the socket is rebuilt (REQ/REP state can become
        corrupted after a timeout) and a RuntimeError is raised so the
        caller can handle the failure cleanly.
        """
        try:
            ...
            raise TimeoutError(
                f"[Env] C++ sim did not reply within {self._timeout} ms. "
                "Ensure simulator.cpp is running and ready."
            )
